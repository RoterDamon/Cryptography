...
namespace First
{
    class Encryption : ThirdInterface2
    {
        #region
        ...
        public byte[] EncryptFunc(byte[] block, byte[] roundKey)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                B = (ulong)(expandedN >> (i * 6)) & ((uint)1 << 6) - 1;// последовательные 8 блоков по 6 бит
                row = ((B >> 5) << 1) | B & 1;// первый и последний 
                column = B >> 1 & 15;// средние 4 разряда
        ...
        {
            ulong res = 0;
            ulong n = BitConverter.ToUInt32(block);
